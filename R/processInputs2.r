...
#'
#' @examples
#' self <- selfSummary$new(species = c("largeresident", "jackchinook", "sockeye", "adultchinook"), site = "Mission", estDate = "2023-08-05")
#' 
#' @export
speciesCompModel <- R6Class("SpeciesComp",
  public = list(
    ...
    plot = function(day = 1, include_proportion_labels = FALSE, ...){
      plot_mix(self, day = day, include_proportion_labels = include_proportion_labels, ...)
    },
    plotBeamSpreading = function(self){
...
default_params <- function(){
  ...
  default_parameters$mu <- c("smallresident" = 23, "largeresident" = 35, "pink" = 50, "sockeye" = 58, "jackchinook" = 40, "smalladultchinook" = 62, "largeadultchinook" = 80, "adultchinook" = 70)
  default_parameters$sigma <- c("smallresident" = 2.5, "largeresident" = 2.5, "pink" = 4, "sockeye" = 3.5, "jackchinook" = 2.5, "smalladultchinook" = 4, "largeadultchinook" = 6, "adultchinook" = 7)
  default_parameters$sigma0 <- 4.5
  default_parameters$delta_mu <- c("smallresident" = 0, "largeresident" = 0, "pink" = 0, "sockeye" = 0, "jackchinook" = 0, "smalladultchinook" = 0, "largeadultchinook" = 0, "adultchinook" = 0)
...
set_model_parameters <- function(self, fixed_parameters = c("mu", "sigma", "proportion_jackchinook"),
  ...
  ## Make sure the daily test fishery data matches the test_fishery_input:
  # if(is.null("test_fishery_spp")){
    # if(self$data_info$site == "Qualark") test_fishery_spp <- c("sockeye_qualark", "chinook_qualark")
    # if(self$data_info$site == "Mission") test_fishery_spp <- c("sockeye_whonnock", "chinook_whonnock", "chinook_albion")
    
  # }
  # self$data_list$test_fishery_catch <- self$data_list$test_fishery_catch |> subset(gsub(paste(species, fishery, sep = "_") %in% test_fishery_spp))
...
process_albion_catch <- function(self, albion_catch){
  ...
  albion_total <- albion_catch |> aggregate(CATCH_QTY~CRPT_DTT+NET_CONFIG, sum)
  ...
  albion_set <- albion_set |> aggregate(cbind(effort, time, net_length) ~ CRPT_DTT + NET_CONFIG, sum)

  albion_total <- albion_total |> merge(albion_set, by = c("CRPT_DTT", "NET_CONFIG"))
...
# predictSpeciesComposition = function(self, proportionOffshore = NULL, salmonPassageTable = NULL, extrapolateBin = "Bin3", catchabilityCorrection = FALSE){
  # if(is.null(self$salmonCounts) ){
    # if(is.null(salmonPassageTable) & self$data_info$site == "Mission") stop("A salmonPassageTable needs to be provided to compute species composition at Mission.")
    # if(self$data_info$site == "Mission") processMissionSalmonPassage(salmonPassageTable)
  # }
  # if(!any(self$estDate %in% self$estimatedDailyProportions$Date)) stop("Must run 'fitModel' before trying to predict species composition for this date.")
  # estimatedP <- self$estimatedDailyProportions
  # estimatedP$SonarCode <- ifelse(estimatedP$SonarBank == "Left Bank", "A1", "A2")

  # estSpp <- self$species
  # if( any(self$species == "smalladultchinook") ){
    # estimatedP$adultchinook <- estimatedP$smalladultchinook + estimatedP$largeadultchinook
    # estimatedP$smalladultchinook <- NULL
    # estimatedP$largeadultchinook <- NULL
  ...
  # ntest <- self$analysisData$test_fishery_counts
  # if(is.null(ntest)){
    # if(is.null(self$test_fishery_counts)) stop("Must provide test fishery counts or 'proportionOffshore' values.")
    # ntest <- self$test_fishery_counts |> subset(Date %in% unique(estimatedP$Date), select = grep("resident", estSpp, invert = TRUE, value = TRUE))
  # }
  # ptest <- data.frame(t(apply(ntest, 1, FUN = function(x){x/sum(x)})))
  # if(catchabilityCorrection){
    # q <- c(exp(self$estimatedParameters$logq), 1)    
    # pcor <- ptest/q
    # ptest <- pcor/sum(pcor)
  # }
  
  # ptest <- cbind(Date = unique(estimatedP$Date), SonarBank = "Offshore", SonarBin = NA, SonarAim = NA, 
    # day = 1:self$ndays, Count = NA, SalmonCount = NA, NHour = 1, smallresident = 0, largeresident = 0, ptest, SonarCode = "mobile")
  # estimatedP <- rbind(estimatedP, ptest[, names(estimatedP)])

  # salmon <- self$salmonCounts |> subset(Date %in% estimatedP$Date)
...
# plotBeamSpreading <- function(self){
  # if(!any(self$estDate %in% self$estimatedDailyProportions$Date)) 
    # stop("Must run 'fitModel' before trying to predict species composition for this date.")
  
  # beta <- self$estimatedParameters$beta
  # self$analysisData$lengthData$L.cm.modadj <- self$analysisData$lengthData$L.cm.adj - self$analysisData$Xlength %*% beta
  # Xnew <- data.frame(R.m = 1:30, SonarBin = rep(c("Bin1", "Bin2", "Bin3"), each = 10))
  # Xnew <- Xnew |> within(beamWidth.cm <- R.m*0.3*pi/180*100)
  # Xnew <- Xnew |> filter(SonarBin %in% unique(self$analysisData$lengthData$SonarBin))
  # predmat <- model.matrix(self$adjustFormula, data = Xnew)
  # Xnew$adjust <- (predmat %*% beta)[,1]
  
  # Now adjust species lengths:
  # mu <- self$estimatedParameters$mu
  # muChin <- self$estimatedParameters$muChin
  # spp <- grep('chinook', self$species, value = TRUE, invert = TRUE)
  # spp2 <- grep('chinook', self$species, value = TRUE, invert = FALSE)
  ...
  # }
  # legend("topleft", legend = speciesLabels(self$species), col = speciesColours(self$species), 
          # lty = rep(1, length(self$species)), ncol = 2)
# }
