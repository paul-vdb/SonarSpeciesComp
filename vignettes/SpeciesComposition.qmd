...
Access to the Qualark database. Creating access can be done by using the R package `metasalmon` to setup a personal access token (PAT) to the GitHub repository and download the latest data on the fly. ***I will make another option for running this without the database later***.
...
# If you have not already set up a connection, then run:
ms_setup_github(repo = "dfo-pacific-science/qualark")
```
...
At Mission, hydroacoustic lengths are taken for 20 fish, every three hours in each sonar Aim, Bin, and Bank. Sonar bins 1-3 are measured, with distances further than that unreliable for length measurement. Similarly, at Qualark, measurement lengths follow the same process but for 2 distance bins, and a single aim due to a target roll angle of 35 degrees. Updates to the sampling protocols will be trialed in 2026.
...
As before, the database connection is required to run the Qualark model.
...
$$
such that $p_1 + p_2 + p_3 = 1$. For estimation purposes, we reformulate this as,

$$
  f(x|z=K) = pf(x|\text{jack}) + (1-p)(af(x|\text{smalladult}) + (1-a)f(x|\text{largeadult})),
$$
where $a$ is the proportion of small adults of adult Chinook in the population based on the test fishery data, or prior knowledge. It is not a parameter to be estimated in the model. However, $p$, the proportion of Chinook that are jack can either be fitted or fixed based on the test fishery data or prior knowledge. We think it is much more likely that we can strongly set a prior on $p$ that is biologically relevant than relative to the non-Chinook, given that we don't expect more than 10\% of Chinook to be jack at any time. The amount of large Chinook then help us differentiate jack Chinook from large resident fish or pink salmon.
...
```
Note that `beamWidth.cm` is a column generated automatically when calling `processData`. To adjust by sonar bin, the user may also choose:
```{r, echo = TRUE}
...
```{r, echo = TRUE}
speciesComp$controlOptimization(verbose = TRUE, tolerance = 1e-8)
```
...
::: {}
1. salmon passage by bin next input to create daily estimates (inputs are: how to do offshore (tangled/gilled) etc., plus how to expand from bin 2/3). Chinook Pink spread sheet.
1. clipboard copy of estimates from markdown report.
1. Check out impact of bin 2 depth profile on fish passage at distance. (e.g. make a nice plot showing the position of the sonar and the relative fish passage at each bin on each sonar). Sonar position is in column B or Cross River Flux.
:::
