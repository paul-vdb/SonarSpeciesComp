...
1. Control optimization for the EM algorithm `setPriors`.
...
Here is where we set the actual analysis data for a specific date. The function `setModelProportions` allows us to control the linear model for modelling the covariate effect on species composition. By default we assume that they are all the same.
...
Can include Jacobians if we want posterior mode. For me there is some preference to do this...
